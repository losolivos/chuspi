...
class boss_zorlok : public CreatureScript
{
    public:
        ...
        {
            boss_zorlokAI(Creature* creature) : BossAI(creature, DATA_VIZIER)
            {
            ...
            void Reset()
            {
                ...
                if (pInstance)
                {
                    EncounterState bossState = pInstance->GetBossState(DATA_VIZIER);
                    if (bossState != DONE && bossState != NOT_STARTED)
                        pInstance->SetBossState(DATA_VIZIER, NOT_STARTED);
                }
                numPlat = 0;
                phase = 0;
                platformToUse = 0;
            ...
            void JustDied(Unit* /*who*/)
            {
                ...
                if (pInstance)
                {
                    pInstance->SetBossState(DATA_VIZIER, DONE);
            ...
            void JustReachedHome()
            {
                ...
                if (pInstance)
                    pInstance->SetBossState(DATA_VIZIER, FAIL);
            }
            ...
            void DamageTaken(Unit* attacker, uint32 &damage)
            {
                ...
                if (!isActive)
                {
                    ...
                    if (!isEcho)
                    {
                        ...
                        phase = PHASE_ZORLOK1;
                        if (pInstance)
                            pInstance->SetBossState(DATA_VIZIER, IN_PROGRESS);
                    }
                }
