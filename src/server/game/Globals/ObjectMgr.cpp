...
uint32 getReverse(uint32 primary)
{
    switch (primary)
    {
...
void ObjectMgr::LoadItemSpecialisation()
{
    ...
    for (auto i = itemStore.begin(); i != itemStore.end(); ++i)
    {
        ...
        ItemTemplate const* sparse = GetItemTemplate(95627);
        for (uint32 x = 0; x < sItemSpecStore.GetNumRows(); x++)
        {
            if (ItemSpecEntry const* entry = sItemSpecStore.LookupEntry(x))
            {
                // levels
                //if (uint32(sparse->RequiredLevel) < entry->LevelMin || uint32(sparse->RequiredLevel) > entry->LevelMax)
                //    continue;

                int32 stats[6] = { 0, 0, 0, 0, 0, 0 };
                bool _p[6] = { false, false, false, false, false, false };

                // inv types
                if (sparse->InventoryType != entry->InventoryType)
                    continue;
                ...
                switch (entry->Group)
                {
                    case 1:
                    {
                        if (sparse->SubClass != ITEM_SUBCLASS_ARMOR_CLOTH)
                            _failed = true;
                        break;
                    }
                    case 2:
                    {
                        if (sparse->SubClass != ITEM_SUBCLASS_ARMOR_LEATHER)
                            _failed = true;
                        break;
                    }
                    case 3:
                    {
                        if (sparse->SubClass != ITEM_SUBCLASS_ARMOR_MAIL)
                            _failed = true;
                        break;
                    }
                    case 4:
                    {
                        if (sparse->SubClass != ITEM_SUBCLASS_ARMOR_PLATE)
                        break;
                    }
                }

                if (_failed && (sparse->InventoryType == ITEM_CLASS_ARMOR))
                    continue;

                for (uint8 stat = 0; stat < MAX_ITEM_PROTO_STATS; stat++)
                {
                    if (sparse->ItemStat[stat].ItemStatType != ITEM_MOD_AGILITY
                        && sparse->ItemStat[stat].ItemStatType != ITEM_MOD_STRENGTH
                        && sparse->ItemStat[stat].ItemStatType != ITEM_MOD_INTELLECT)
                        continue;

                    if (sparse->ItemStat[stat].ItemStatValue <= 0)
                        continue;

                    _p[sparse->ItemStat[stat].ItemStatType] = true;
                    stats[sparse->ItemStat[stat].ItemStatType] = sparse->ItemStat[stat].ItemStatValue;
                }

                // stats
                int32 max = *std::max_element(stats, stats + 6);
                for (uint8 x = 0; x < 6; x++)
                {
                    if (max == stats[x] && getReverse(x) == entry->PrimaryStat && _p[x])
                        TC_LOG_ERROR("general", "(%u, %u) Max stat type: %i /%i - %i - %i/ (%u) - Inv type: %u (%u); Level - %u (%u - %u)", sparse->ItemId, entry->Spec, max, stats[3], stats[4], stats[5], entry->PrimaryStat, sparse->InventoryType, entry->InventoryType, sparse->RequiredLevel, entry->LevelMin, entry->LevelMax);

                    TC_LOG_ERROR("general", "%i, %i, %i, %i, %i, %i - Max: %i - reverse %u . primary %u", stats[0], stats[1], stats[2], stats[3], stats[4], stats[5], max, getReverse(x), entry->PrimaryStat);
                }
            }
        }
        Sleep(30000);
...
void ObjectMgr::LoadLFRLootBinds()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT `id`, `type`, `entry`, `linkGroup`, `lootGroup` FROM `lfr_loot_binds`");
    if (!result)
    {
    ...
    do
    {
        ...
        uint8 lootGroup = fields[4].GetUInt8();
        ...
        if (lootGroup)
            m_lfrLootGroupsLinked[lootGroup].insert(lootBind);
...
LFRLootBindSet const& ObjectMgr::GetLFRLootBindGroup(uint8 lootGroup)
{
    return m_lfrLootGroupsLinked[lootGroup];
}
