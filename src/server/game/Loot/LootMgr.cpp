...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        void GetCompatibleLFRItems(LootStoreItemList &compatibleItems, uint32 specialisation) const;
...
uint32 LootStore::LoadLootTable()
{
    ...
    do
    {
        ...
        // Determine specs for this item
        auto const& itemSpecs = sObjectMgr->GetItemSpecialisations();
        if (itemSpecs.find(item) != itemSpecs.end())
            for (auto itr: itemSpecs)
                if (itr.first == item)
                    for (auto _itr: itr.second)
                        storeitem.specs.push_back(_itr->Specialisation);
...
void LootTemplate::LootGroup::GetCompatibleLFRItems(LootStoreItemList &compatibleItems, uint32 specialisation) const
{
    // find items that are compatible with the class specialisation
    for (auto const lootItem : EqualChanced)
    {
        if (IsEligibleForLFRLootItem(lootItem, specialisation))
            compatibleItems.push_back(lootItem);
    }
}
...
{
    auto specList = item.specs;
    for (auto itr: specList)
        if (itr == specialisation)
            return true;
...
void LootTemplate::ProcessLFRItem(Loot& loot, uint32 specialisation) const
{
    ...
    // find all compatible items from loot table for the class specialisation
    LootStoreItemList compatibleItems;
    for (auto const &lootGroup : Groups)
        lootGroup.GetCompatibleLFRItems(compatibleItems, specialisation);

    if (compatibleItems.size())
    {
        // select a random item that can be used by the class specialisation
        uint32 itemIndex = urand(0, compatibleItems.size() - 1);

        loot.AddItem(compatibleItems[itemIndex]);
