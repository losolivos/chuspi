...
bool ItemChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): linked item has color %u, but user claims %u", iss.str().c_str(), ItemQualityColors[_item->Quality], _color);
        return false;
    }
    ...
    for (uint8 index = 0; index < propsCount; ++index)
    {
        ...
        if (!ReadInt32(iss, id))
        {
            TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item property (%u)", iss.str().c_str(), index);
            return false;
        }
        if (id && (index == randomPropertyPosition))
        {
            ...
            if (id > 0)
            {
                ...
                if (!_property)
                {
                    TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid item property id %u in |item command", iss.str().c_str(), id);
                    return false;
                }
            }
            else if (id < 0)
            {
                ...
                if (!_suffix)
                {
                    TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid item suffix id %u in |item command", iss.str().c_str(), -id);
                    return false;
                }
            }
        }
...
bool ItemChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (!res)
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): linked item (id: %u) name wasn't found in any localization", context, _item->ItemId);
    return res;
}
...
bool QuestChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): quest level %d is too big", iss.str().c_str(), _questLevel);
        return false;
    }
    return true;
}
...
bool SpellChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (_spell->Attributes & SPELL_ATTR0_TRADESPELL)
    {
        ...
        if (!skillLine)
        {
            TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): skill line not found for skill %u", context, skillInfo->skillId);
            return false;
        }
...
bool AchievementChatLink::Initialize(std::istringstream& iss)
{
    ...
    if (!_achievement)
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid achievement id %u in |achievement command", iss.str().c_str(), achievementId);
        return false;
    }
    // Check delimiter
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): invalid hexadecimal number while reading char's guid", iss.str().c_str());
        return false;
    }
    // Skip progress
...
bool TradeChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading minimum talent level", iss.str().c_str());
        return false;
    }
    // Check delimiter
    if (!CheckDelimiter(iss, DELIMITER, "trade"))
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading maximum talent level", iss.str().c_str());
        return false;
    }
    // Check delimiter
    if (!CheckDelimiter(iss, DELIMITER, "trade"))
    ...
    if (!ReadHex(iss, _guid, 0))
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading achievement's owner guid", iss.str().c_str());
        return false;
    }
...
bool TalentChatLink::Initialize(std::istringstream& /*iss*/)
{
    ...
    if (!_spell)
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |trade command", iss.str().c_str(), talentInfo->RankID[0]);
        return false;
    }
    // Delimiter
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading talent rank", iss.str().c_str());
        return false;
    }
    return true;*/
}
...
bool GlyphChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading slot id", iss.str().c_str());
        return false;
    }
    // Check delimiter
    ...
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading glyph entry", iss.str().c_str());
        return false;
    }
    // Validate glyph
    ...
    if (!_glyph)
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid glyph id %u in |glyph command", iss.str().c_str(), glyphId);
        return false;
    }
    ...
    if (!_spell)
    {
        TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |glyph command", iss.str().c_str(), _glyph->SpellId);
        return false;
    }
    return true;
}
...
bool LinkExtractor::IsValidMessage()
{
    ...
    while (!_iss.eof())
    {
        ...
        {
            TC_LOG_DEBUG("chat.system", "ChatHandler::isValidChatMessage('%s'): sequence aborted unexpectedly", _iss.str().c_str());
            return false;
        }
