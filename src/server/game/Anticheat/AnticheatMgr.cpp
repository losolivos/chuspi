...
void AnticheatMgr::SpeedHackDetection(Player* player,MovementInfo movementInfo)
{
	...
	if (distance2D < 100 && player->IsInCombat())
	    return;
	...
	if ( player->GetMap()->IsDungeon() || player->GetMap()->IsRaid() || player->GetMap()->IsBattlegroundOrArena())
		if (!player->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) && player->IsAlive())
		{
			...
			{
				if ((distance2D > 20 && player->GetMapId() == 566) || (distance2D > 20 && player->GetMapId() != 566))
					player->TeleportTo(player->GetMapId(),player->GetPositionX(),player->GetPositionY(),player->GetPositionZ(),player->GetOrientation());
			}
			else
			{
				if ((distance2D > 7 && player->GetMapId() == 566) || (distance2D > 7 && player->GetMapId() != 566))
					player->TeleportTo(player->GetMapId(),player->GetPositionX(),player->GetPositionY(),player->GetPositionZ(),player->GetOrientation());
			}
			
			if (distance2D > 100)
				player->TeleportTo(530,-1890.4f,5398.62f,-12.42f,4.04241f);
		}
	...
	if (player->IsFlying() && player->HasAuraType(SPELL_AURA_MOUNTED))
		speedRate *= 2;
