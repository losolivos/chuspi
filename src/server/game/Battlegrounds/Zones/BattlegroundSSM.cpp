                    ...
                    case BG_SSM_MINE_CART_1:
                    {
                        if (trigger)
                        {
                            ...
                            if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_1, 99999.0f))
                            {
                                ...
                                for (uint8 i = 0; i < 2; i++)
                                    if (Creature* visual = cart->SummonCreature(NPC_MINE_CART_1_H + i, cart->GetPosition()))
                                    {
                                        visual->SetVisible(false);
                                        visual->SetUnitMovementFlags(MOVEMENTFLAG_BACKWARD);
                                        visual->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                                        visual->SetSpeed(MOVE_WALK, 0.8f);
                                    }
                            }
                        }
                        break;
                    ...
                    case BG_SSM_MINE_CART_2:
                    {
                        if (trigger)
                        {
                            ...
                            if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_2, 99999.0f))
                            {
                                ...
                                for (uint8 i = 0; i < 2; i++)
                                    if (Creature* visual = cart->SummonCreature(NPC_MINE_CART_2_H + i, cart->GetPosition()))
                                    {
                                        visual->SetVisible(false);
                                        visual->SetUnitMovementFlags(MOVEMENTFLAG_BACKWARD);
                                        visual->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                    ...
                    case BG_SSM_MINE_CART_3:
                    {
                        if (trigger)
                        {
                            ...
                            if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_3, 99999.0f))
                            {
                                ...
                                for (uint8 i = 0; i < 2; i++)
                                    if (Creature* visual = cart->SummonCreature(NPC_MINE_CART_3_H + i, cart->GetPosition()))
                                    {
                                        visual->SetVisible(false);
                                        visual->SetUnitMovementFlags(MOVEMENTFLAG_BACKWARD);
                                        visual->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                ...
                case BG_SSM_MINE_CART_1:
                {
                    if (trigger)
                    {
                        ...
                        if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_1, 99999.0f))
                        {
                            ...
                            for (uint8 i = 0; i < 2; i++)
                                if (Creature* visual = trigger->SummonCreature(NPC_MINE_CART_1_H + i, cart->GetPosition()))
                                {
                                    visual->SetVisible(false);
                                    visual->SetUnitMovementFlags(MOVEMENTFLAG_BACKWARD);
                                    visual->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                ...
                case BG_SSM_MINE_CART_2:
                {
                    if (trigger)
                    {
                        ...
                        if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_2, 99999.0f))
                        {
                            ...
                            for (uint8 i = 0; i < 2; i++)
                                if (Creature* visual = trigger->SummonCreature(NPC_MINE_CART_2_H + i, cart->GetPosition()))
                                {
                                    visual->SetVisible(false);
                                    visual->SetUnitMovementFlags(MOVEMENTFLAG_BACKWARD);
                                    visual->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                ...
                case BG_SSM_MINE_CART_3:
                {
                    if (trigger)
                    {
                        ...
                        if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_3, 99999.0f))
                        {
                            ...
                            for (uint8 i = 0; i < 2; i++)
                                if (Creature* visual = trigger->SummonCreature(NPC_MINE_CART_3_H + i, cart->GetPosition()))
                                {
                                    visual->SetVisible(false);
                                    visual->SetUnitMovementFlags(MOVEMENTFLAG_BACKWARD);
                                    visual->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            ...
            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
            {
                ...
                if (player->GetTeam() == ALLIANCE)
                {
                    if (Creature* cart = player->FindNearestCreature(NPC_MINE_CART_1, 22.0f, true))
                    {
                        ...
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_1_A, 3.0f, true))
                            {
                                if (!visual->HasAura(BG_SSM_CONTROL_VISUAL_ALLIANCE))
                                {
                                    visual->CastSpell(visual, BG_SSM_CONTROL_VISUAL_ALLIANCE, true);
                        ...
                        if (m_MineCartsProgressBar[BG_SSM_MINE_CART_1 - 1] == BG_SSM_PROGRESS_BAR_NEUTRAL)
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_1_A, 3.0f, true))
                            {
                                if (visual->HasAura(BG_SSM_CONTROL_VISUAL_ALLIANCE))
                                {
                                    visual->RemoveAurasDueToSpell(BG_SSM_CONTROL_VISUAL_ALLIANCE);
                                    visual->SetVisible(false);
                    ...
                    else if (Creature* cart = player->FindNearestCreature(NPC_MINE_CART_2, 22.0f, true))
                    {
                        ...
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_2_A, 3.0f, true))
                            {
                                if (!visual->HasAura(BG_SSM_CONTROL_VISUAL_ALLIANCE))
                                {
                                    visual->CastSpell(visual, BG_SSM_CONTROL_VISUAL_ALLIANCE, true);
                    ...
                    else if (Creature* cart = player->FindNearestCreature(NPC_MINE_CART_3, 22.0f, true))
                    {
                        ...
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_3_A, 3.0f, true))
                            {
                                if (!visual->HasAura(BG_SSM_CONTROL_VISUAL_ALLIANCE))
                                {
                                    visual->CastSpell(visual, BG_SSM_CONTROL_VISUAL_ALLIANCE, true);
                ...
                else // for GetTeam() == HORDE
                {
                    if (Creature* cart = player->FindNearestCreature(NPC_MINE_CART_1, 22.0f, true))
                    {
                        ...
                        if (m_MineCartsProgressBar[BG_SSM_MINE_CART_1 - 1] < BG_SSM_PROGRESS_BAR_NEUTRAL)
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_1_H, 3.0f, true))
                            {
                                if (!visual->HasAura(BG_SSM_CONTROL_VISUAL_HORDE))
                                {
                                    visual->CastSpell(visual, BG_SSM_CONTROL_VISUAL_HORDE, true);
                                    visual->SetVisible(true);
                                    cart->SetVisible(false);
                        ...
                        if (m_MineCartsProgressBar[BG_SSM_MINE_CART_1 - 1] == BG_SSM_PROGRESS_BAR_NEUTRAL)
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_1_H, 3.0f, true))
                            {
                                if (visual->HasAura(BG_SSM_CONTROL_VISUAL_HORDE))
                                {
                                    visual->RemoveAurasDueToSpell(BG_SSM_CONTROL_VISUAL_HORDE);
                                    visual->SetVisible(false);
                                    cart->SetVisible(true);
                                }
                            }
                    ...
                    else if (Creature* cart = player->FindNearestCreature(NPC_MINE_CART_2, 22.0f, true))
                    {
                        ...
                        if (m_MineCartsProgressBar[BG_SSM_MINE_CART_2 - 1] < BG_SSM_PROGRESS_BAR_NEUTRAL)
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_2_H, 3.0f, true))
                            {
                                if (!visual->HasAura(BG_SSM_CONTROL_VISUAL_HORDE))
                                {
                                    visual->CastSpell(visual, BG_SSM_CONTROL_VISUAL_HORDE, true);
                    ...
                    else if (Creature* cart = player->FindNearestCreature(NPC_MINE_CART_3, 22.0f, true))
                    {
                        ...
                        if (m_MineCartsProgressBar[BG_SSM_MINE_CART_3 - 1] < BG_SSM_PROGRESS_BAR_NEUTRAL)
                        {
                            if (Unit* visual = cart->FindNearestCreature(NPC_MINE_CART_3_H, 3.0f, true))
                            {
                                if (!visual->HasAura(BG_SSM_CONTROL_VISUAL_HORDE))
                                {
                                    visual->CastSpell(visual, BG_SSM_CONTROL_VISUAL_HORDE, true);
...
void BattlegroundSSM::MineCartsMoves(uint32 diff)
{
    ...
    if (trigger = GetBgMap()->GetCreature(BgCreatures[SSM_MINE_CART_TRIGGER]))
    {
        if (!m_PathDone[SSM_EAST_PATH][0])
        {
            ...
            for (uint8 i = 0; i < 2; i++)
                if (Creature* visual = trigger->FindNearestCreature(NPC_MINE_CART_1_H + i, 99999.0f, true))
                    visual->GetMotionMaster()->MovePath(NPC_MINE_CART_1, false);
        }
                ...
                if (cart->GetExactDist2d(717.169312f, 114.258339f) < 0.5f) // East pos
                    if (Creature* track = GetBgMap()->GetCreature(BgCreatures[SSM_TRACK_SWITCH_EAST]))
                    {
                        ...
                        for (uint8 i = 0; i < 2; i++)
                            if (Creature* visual = trigger->FindNearestCreature(NPC_MINE_CART_1_H + i, 99999.0f, true))
                            {
                                visual->GetMotionMaster()->Clear(true);
                                visual->GetMotionMaster()->MovePath(track->HasAura(BG_SSM_TRACK_SWITCH_OPENED) ? NPC_MINE_CART_1 * 10 : NPC_MINE_CART_1 * 100, false);
                            }
                    }
        ...
        if (!m_WaterfallPathDone)
        {
            if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_2, 99999.0f, true))
            {
                ...
                for (uint8 i = 0; i < 2; i++)
                    if (Creature* visual = trigger->FindNearestCreature(NPC_MINE_CART_2_H + i, 99999.0f, true))
                    {
                        visual->GetMotionMaster()->Clear(true);
                        visual->GetMotionMaster()->MovePath(NPC_MINE_CART_2, false);
                    }
            }
        }
        ...
        if (!m_PathDone[SSM_NORTH_PATH][0])
            if (Creature* cart = trigger->FindNearestCreature(NPC_MINE_CART_3, 99999.0f, true))
            {
                ...
                for (uint8 i = 0; i < 2; i++)
                    if (Creature* visual = trigger->FindNearestCreature(NPC_MINE_CART_3_H + i, 99999.0f, true))
                    {
                        visual->GetMotionMaster()->Clear(true);
                        visual->GetMotionMaster()->MovePath(NPC_MINE_CART_3, false);
                    }
            }
                ...
                if (cart->GetExactDist2d(834.727234f, 299.809753f) < 0.5f) // North pos
                    if (Creature* track = GetBgMap()->GetCreature(BgCreatures[SSM_TRACK_SWITCH_NORTH]))
                    {
                        ...
                        for (uint8 i = 0; i < 2; i++)
                            ...
                            {
                                visual->GetMotionMaster()->Clear(true);
                                visual->GetMotionMaster()->MovePath(track->HasAura(BG_SSM_TRACK_SWITCH_OPENED) ? NPC_MINE_CART_3 * 10 : NPC_MINE_CART_3 * 100, false);
                            }
                    }
    }
}
