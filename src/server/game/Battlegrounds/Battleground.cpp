...
void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
    {
        ...
        if (isArena())
        {
            if (GetPlayersCountByTeam(ALLIANCE) == 0 || GetPlayersCountByTeam(HORDE) == 0)
...
void Battleground::RewardReputationToTeam(uint32 a_faction_id, uint32 h_faction_id, uint32 Reputation, uint32 TeamID)
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (!plr)
        {
            TC_LOG_ERROR("general", "BattleGround:RewardReputationToTeam: %u not found!", itr->first);
            continue;
        }
...
void Battleground::AddPlayer(Player* player)
{
    ...
    if (isArena())
    {
        ...
        player->SetByteValue(PLAYER_BYTES_3, 3, player->GetTeam() == HORDE ? 0 : 1);

        WorldPacket team_packet(SMSG_ARENA_OPPONENT_SPECIALIZATIONS);
        BuildArenaOpponentSpecializations(&team_packet, player->GetTeam());
        SendPacketToTeam(GetOtherTeam(player->GetTeam()), &team_packet, player, false);

        team_packet.Initialize(SMSG_ARENA_OPPONENT_SPECIALIZATIONS);
        BuildArenaOpponentSpecializations(&team_packet, GetOtherTeam(player->GetTeam()));
